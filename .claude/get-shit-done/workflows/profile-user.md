...
```bash
cp "/workspace/.claude/get-shit-done/USER-PROFILE.md" "/workspace/.claude/get-shit-done/USER-PROFILE.backup.md"
```
...
Run session scan:
```bash
SCAN_RESULT=$(node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Get questions:**
```bash
QUESTIONS=$(node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="/workspace/.claude/get-shit-done/USER-PROFILE.backup.md"
```
