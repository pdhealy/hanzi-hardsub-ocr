...
```bash
node "/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node "/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
